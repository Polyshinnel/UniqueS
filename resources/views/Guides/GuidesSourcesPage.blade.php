...
<div class="sources-container">
    <div class="breadcrumb">
        <a href="/">Главная</a> / <a href="/guide">Справочники</a> / Источники
    </div>
    @if(session('success'))
                ...
                <tr class="source-row">
                    ...
                    <td class="actions-cell">
                        <div class="action-buttons">
                            <button class="action-btn edit-btn" onclick="openEditModal({{ $source->id }})" title="Редактировать">
                ...
                <tr>
                    <td colspan="3" class="empty-state">
                        <div class="empty-content">
                            ...
                            <button class="btn btn-primary" onclick="openModal()">Добавить первый источник</button>
                        </div>
                    </td>
                </tr>
                @endforelse
...
<!-- Модальное окно для добавления источника -->
<div id="sourceModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Добавление источника</h3>
        ...
        <div class="modal-body">
            <form action="{{ route('sources.store') }}" method="POST" class="source-form">
                @csrf
...
<!-- Модальное окно для редактирования источника -->
<div id="editSourceModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Редактирование источника</h3>
        ...
        <div class="modal-body">
            <form id="editSourceForm" method="POST" class="source-form">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="edit_name">Название источника *</label>
...
/* Стили для ячейки с информацией об источнике */
.source-info-cell {
    min-width: 300px;
}
...
function resetForm() {
    ...
    form.reset();
    const errorMessages = document.querySelectorAll('#sourceModal .error-message');
    errorMessages.forEach(error => error.remove());
    const errorInputs = document.querySelectorAll('#sourceModal .form-control.is-invalid');
...
function openEditModal(sourceId) {
    ...
    fetch(`/guide/sources/${sourceId}/edit`)
        .then(response => response.json())
        .then(source => {
            // Заполняем форму данными
            document.getElementById('edit_name').value = source.name;
            document.getElementById('edit_active').value = source.active ? '1' : '0';
            
            // Устанавливаем action для формы
            document.getElementById('editSourceForm').action = `/guide/sources/${sourceId}`;
...
function resetEditForm() {
    const form = document.querySelector('#editSourceForm');
    form.reset();
    // Очищаем ошибки валидации
    const errorMessages = document.querySelectorAll('#editSourceModal .error-message');
    errorMessages.forEach(error => {
        error.style.display = 'none';
        error.textContent = '';
    });
    const errorInputs = document.querySelectorAll('#editSourceModal .form-control.is-invalid');
    errorInputs.forEach(input => input.classList.remove('is-invalid'));
}
