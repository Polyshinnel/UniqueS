...
class ProductHoldStatusTest extends TestCase
{
    use RefreshDatabase;
    ...
    private function createTestData()
    {
        ...
        ProductStatus::create(['name' => 'Холд', 'active' => true]);
    ...
    public function test_product_hold_status_creates_task()
    {
        ...
        $this->assertEquals('Холд', $this->product->status->name);
        ...
        $this->assertEquals(now()->addMonths(3)->format('Y-m-d'), $productAction->expired_at->format('Y-m-d'));
    }

    public function test_product_hold_status_updates_advertisements()
    {
        $holdStatus = ProductStatus::where('name', 'Холд')->first();
        ...
        // Проверяем, что активное объявление перешло в статус Холд
        $this->activeAdvertisement->refresh();
        $this->assertEquals('Холд', $this->activeAdvertisement->status->name);
    ...
    public function test_product_hold_status_creates_advertisement_logs()
    {
        $holdStatus = ProductStatus::where('name', 'Холд')->first();
        ...
        $advLog = AdvLog::where('advertisement_id', $this->activeAdvertisement->id)
            ->where('log', 'В связи с переводом товара в статус Холд, объявление переводится в статус Холд.')
            ->first();
    ...
    public function test_product_hold_status_creates_advertisement_tasks()
    {
        $holdStatus = ProductStatus::where('name', 'Холд')->first();
        ...
        $this->assertEquals(now()->addMonths(3)->format('Y-m-d'), $advAction->expired_at->format('Y-m-d'));
    ...
    public function test_product_hold_status_creates_product_log()
    {
        $holdStatus = ProductStatus::where('name', 'Холд')->first();
        ...
        $productLog = ProductLog::where('product_id', $this->product->id)
            ->where('log', 'Смена статуса товара с \'В работе\' на \'Холд\'. Комментарий: Тестовый комментарий')
            ->first();
