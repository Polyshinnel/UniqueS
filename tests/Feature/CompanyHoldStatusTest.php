...
class CompanyHoldStatusTest extends TestCase
{
    use RefreshDatabase;

    protected $company;
    protected $user;
    protected $holdCompanyStatus;
    protected $holdProductStatus;
    protected $holdAdvertisementStatus;
    ...
    protected function setUp(): void
    {
        ...
        // Создаем статусы
        $this->holdCompanyStatus = CompanyStatuses::create(['name' => 'Холд']);
        $this->holdProductStatus = ProductStatus::create(['name' => 'Холд']);
        $this->holdAdvertisementStatus = AdvertisementStatus::create(['name' => 'Холд']);
    ...
    /** @test */
    public function it_creates_company_action_when_company_status_changed_to_hold()
    {
        // Выполняем запрос на изменение статуса
        $response = $this->actingAs($this->user)
            ->patchJson("/company/{$this->company->id}/status", [
                'status_id' => $this->holdCompanyStatus->id,
    ...
    /** @test */
    public function it_updates_product_statuses_when_company_status_changed_to_hold()
    {
        ...
        $this->assertDatabaseHas('products', [
            'id' => $product1->id,
            'status_id' => $this->holdProductStatus->id
        ]);
    ...
    /** @test */
    public function it_creates_product_logs_and_actions_when_company_status_changed_to_hold()
    {
        ...
        $this->assertDatabaseHas('product_logs', [
            ...
            'log' => "В связи с переводом компании \"{$this->company->name}\" в статус Холд, товар переводится в статус холд.",
            'user_id' => null
        ]);
    ...
    /** @test */
    public function it_updates_advertisement_statuses_when_company_status_changed_to_hold()
    {
        // Создаем товар для компании
        $product = Product::factory()->create([
            'company_id' => $this->company->id,
        ...
        $this->assertDatabaseHas('advertisements', [
            ...
            'status_id' => $this->holdAdvertisementStatus->id
        ]);
    ...
    /** @test */
    public function it_creates_advertisement_logs_and_actions_when_company_status_changed_to_hold()
    {
        // Создаем товар для компании
        $product = Product::factory()->create([
            'company_id' => $this->company->id,
        ...
        $this->assertDatabaseHas('adv_logs', [
            ...
            'type_id' => $this->systemLogType->id,
            'log' => "В связи с переводом компании \"{$this->company->name}\" в статус Холд, объявление переводится в статус Холд.",
            'user_id' => null
        ]);
    ...
    public function it_does_not_affect_products_and_advertisements_in_excluded_statuses()
    {
        ...
        $soldProductStatus = ProductStatus::create(['name' => 'Продано']);
        $holdProductStatus = ProductStatus::create(['name' => 'Холд']);
        ...
        $holdProduct = Product::factory()->create([
            'company_id' => $this->company->id,
            'status_id' => $holdProductStatus->id
        ]);
        ...
        $holdAdvertisementStatus = AdvertisementStatus::create(['name' => 'Холд']);
        ...
        $archiveAdvertisement = Advertisement::factory()->create([
            'product_id' => $holdProduct->id,
            'status_id' => $archiveAdvertisementStatus->id
        ]);

        $holdAdvertisement = Advertisement::factory()->create([
            'product_id' => $refuseProduct->id,
            'status_id' => $holdAdvertisementStatus->id
        ]);
    ...
    public function it_creates_company_log_with_correct_information()
    {
        ...
        $this->assertDatabaseHas('company_logs', [
            'company_id' => $this->company->id,
            'user_id' => $this->user->id,
            'log' => 'Смена статуса с \'В работе\' на \'Холд\'. Комментарий: Тестовый комментарий'
        ]);
    }
} 